...
EEG intensities increase when the eyes are closed and decrease when the eyes are open
...
Yes, there is an obvious temporal correlation between the states of eyes being open and closed. It appears that every eye-open is followed by a equivalent duration of eye-close.
...
Yes, electrodes such as AF3, O2, F8, FC6, F4, AF4, P8 and T8 show consistently higher intensity and variability when the eyes are open.
...
A property that the mean, variance and autocorrelation structure do not change over time.
...
We're interested in stationarity because it allows us to use many statistical and machine learning models. The results of the tests indicate that most of our EEG data is stationary. 
...
FC5,FC6 and O1 show signs of strong autocorrelation
FC5-F7,T8-F4 show signs of cross-correlation
...
When the eyes are closed, the power spectral densities are higher in some channels.
...
Based on the ICA analysis and the generated plot, it appears that at least one independent components do show distinct variations correlating with eye-opening events. Thus, it seems that the process of opening the eyes does indeed relate to an independent component of activity across the electrodes.
...
```{r model2}
# Convert the training and validation datasets to matrices
eeg_train_matrix <- as.matrix(eeg_train[, !(names(eeg_train) %in% c("eyeDetection", "ds"))])
eeg_train_labels <- eeg_train$eyeDetection

eeg_validate_matrix <- as.matrix(eeg_validate[, !(names(eeg_validate) %in% c("eyeDetection", "ds"))])
eeg_validate_labels <- eeg_validate$eyeDetection

# Define the training control
ctrl <- trainControl(method = "cv", number = 5)  # Using 5-fold cross-validation

# Build the GLM model using caret's train function
model2 <- train(x = eeg_train_matrix, 
                y = eeg_train_labels,
                method = "glm", 
                family = "binomial", 
                trControl = ctrl)
...
```{r test}
# Convert the test dataset to a matrix (assuming it's named eeg_test)
eeg_test_matrix <- as.matrix(eeg_test[, !(names(eeg_test) %in% c("eyeDetection", "ds"))])
eeg_test_labels <- eeg_test$eyeDetection

# Use the best performing model to make predictions on the test set
test_predictions <- predict(model2, newdata = eeg_test_matrix)

# Calculate accuracy
accuracy <- sum(test_predictions == eeg_test_labels) / length(eeg_test_labels)
print(paste0("Accuracy on the test set: ", round(accuracy * 100, 2), "%"))
```
