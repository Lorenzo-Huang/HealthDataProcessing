...
```{r}
tokenized.data.sentence <- analysis.data %>%
  tidytext::unnest_tokens(output = sentence, input = transcription, token = "sentences")
...
No. Medical data often contains a vast amount of medical terms and drug names that are derived from Greek or Latin, or some specific abbreviations which might be challenging for a general-purpose lemmatizer to handle.
...
There is a high similarity in lemma frequency between Surgery and Orthopedic specialties, on the other hand, Radiology and Surgery has lower similarity in lemma frequency. It is what I would expect, because Orthopedic is a subfield of Surgery and therefore shares a lot of terminology, whereas Radiology is a more distinct field with its own specific terminology.
...
The lemma 'closed vicryl' appeared on the both Orthopedic and Surgery list, which is due to the overlaps in these two medical specialties.
...
Yes, they still shows up. Specialties are split into sub-topics based on recurring word patterns that are present within each specialty.
...
top.terms.6 <- emr.topics %>% dplyr::group_by(topic) %>% 
  dplyr::slice_max(beta, n=10) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(topic, -beta)
...
```{r}
top_terms_5 <- top.terms$term

# check if these terms are in the top terms of the 6-topic model
top_terms_6 <- top.terms.6$term
common_terms <- top_terms_6[top_terms_6 %in% top_terms_5]
print(common_terms)
```
