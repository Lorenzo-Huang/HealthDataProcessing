...
Bolsa Família: current social welfare program of the Government of Brazil, part of the Fome Zero network of federal assistance programs. Bolsa Família provided financial aid to poor Brazilian families.
...
1. The patient didn't receive notification from hospital.
2. The patient has some disability that require assistance.
3. The transportation from one's residence to hospital is not ideal.
...
1. what type of doctor patient is seeing
2. how urgent the one's medical condition is
3. what is patient's regular/previous hospital
...
SchedulefDate - AppointmentDate
AppointementID - AppointmentDate
Age - Hypertension

**9** Do you see any issues with PatientID/AppointmentID being included in this plot? 
They are unique identifiers and not numerical variables, their correlation is not very meaningful.
...
This might caused by the tendency of sending SMS reminders more often to patients who have a history of missing appointments. 

**12** Create a similar plot which compares the the density of `NoShow` across the values of disability 

```{r}
ggplot(raw.data, fig.align="center") + 
...
Some neighborhood may has fewer hospitals, causing residents have to commute, hence pose hindrance to show up for appointment.
...
raw.data$Lag.days <- as.numeric(as.integer(raw.data$Lag.days))
ggplot(raw.data) + 
...
Lag become too large, above 50, and meanwhile the density become almost 0, seems not very meaningful
...
It might work, because features like 'SMSReceived', 'Disability', and 'Neighbourhood' has certain correlation with 'NoShow', so the model should be able to make somewhat acurate prediction.
...
```{r}
# Define the control using the caret package
fit.control <- trainControl(method = "cv",
                           number = 3,
                           classProbs = TRUE,
                           summaryFunction = twoClassSummary)

logit.grid <- expand.grid(C = c(0.1, 1, 10))

# Fit the Logistic Regression model
set.seed(42)  
logit.model <- train(NoShow ~ ., 
                   data = train, 
                   method = "glm", 
                   family = "binomial",
                   metric = "ROC",
...
# Add a numerical column for NoShow in the test dataset
test <- test %>% mutate(NoShow.numerical = ifelse(NoShow=="Yes",1,0))

# Generate the confusion matrix
logitCM <- confusionMatrix(logit.pred, test$NoShow, positive="Yes")
print(logitCM)

# Calculate Area under ROC Curve
logitAUC <- auc(test$NoShow.numerical, logit.probs[,2])
paste("Logistic Regression Area under ROC Curve: ", round(logitAUC,3))

```

```{r ,fig.align="center"}
logit.probs$Actual = test$NoShow.numerical
logit.probs$ActualClass = test$NoShow
logit.probs$PredictedClass = xgb.pred
logit.probs$Match = ifelse(logit.probs$ActualClass == logit.probs$PredictedClass,
                         "Correct","Incorrect")
# [4.8] Plot Accuracy
logit.probs$Match = factor(logit.probs$Match,levels=c("Incorrect","Correct"))
ggplot(logit.probs,aes(x=Yes,y=Actual,color=Match))+
  geom_jitter(alpha=0.2,size=0.25)+
  scale_color_manual(values=c("grey40","orangered"))+
  ggtitle("Visualizing Model Performance", "(Dust Plot)")
```
**18** Based on everything, do you think we do you think we can trust analyses based on this dataset? Explain your reasoning.

I think we shouldn't trust analyses based on this dataset. First, it missed some critical features like severity of patient's condition, medical history etc. Second, we run 2 models on it, it only has roughly 80% accuracy rate, which can be inproved, suffice we have a more comprehensive data set.
